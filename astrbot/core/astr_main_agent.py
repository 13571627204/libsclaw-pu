...
async def _apply_kb(
    ...
    """Apply knowledge base tools and optional retrieval within persona policy.

    Args:
        event: Current message event.
        req: Provider request being prepared.
        plugin_context: Runtime plugin context.
        config: Normalized main-agent configuration.
        allowed_tool_names: Persona tool allowlist, or ``None`` for all tools.
    """
    ...
    if not config.kb_agentic_mode:
        if (
            ...
            and "astr_kb_search" not in allowed_tool_names
        ):
            return
...
async def _ensure_persona_and_skills(
    ...
    """Apply the selected persona, skills, and non-builtin persona tools.

    Args:
        req: Provider request being prepared.
        cfg: Active provider settings.
        plugin_context: Runtime plugin context.
        event: Current message event.

    Returns:
        The persona tool allowlist, or ``None`` when all configured tools are allowed.
    """
    ...
    if (persona and persona.get("tools") is None) or not persona:
        ...
        if persona["tools"]:
            for tool_name in persona["tools"]:
                if tmgr.is_builtin_tool(tool_name):
                    continue
                tool = tmgr.get_func(tool_name)
                if tool and tool.active:
                    persona_toolset.add_tool(tmgr.guard_tool(tool))
    ...
    main_persona_tools = persona.get("tools") if persona else None
    memory_tool_allowed = (
        main_persona_tools is None or "astr_persona_memory" in main_persona_tools
    )
    if (
        persona
        and persona_id not in {None, "default", "_chatui_default_", "[%None]"}
        and memory_tool_allowed
    ):
    ...
    if orch_cfg.get("main_enable", False) and so:
            ...
            for a in agents:
                ...
                if pid:
                    ...
                    if subagent_persona is not None:
                        subagent_persona_tools = subagent_persona.get("tools")
                tools = a.get("tools", [])
                if subagent_persona_tools is not None:
                    tools = subagent_persona_tools
                if tools is None:
        ...
        for tool in so.handoffs:
            if main_persona_tools is not None and tool.name not in main_persona_tools:
                continue
        ...
        if router_prompt and handoff_added:
            req.system_prompt += f"\n{router_prompt}\n"
    try:
        event.trace.record(
            "sel_persona",
    ...
    return persona.get("tools") if persona else None
...
async def _decorate_llm_request(
    ...
    """Decorate a provider request and return its persona tool policy.
    ...
    Args:
        ...
        plugin_context: Runtime plugin context.
        config: Normalized main-agent configuration.
        provider: Selected chat provider, when available.
    ...
    persona_tools = None
    if req.conversation:
        persona_tools = await _ensure_persona_and_skills(
...
async def build_main_agent(
    *,
    ...
    persona_tools = await _decorate_llm_request(
        event, req, plugin_context, config, provider=provider
    )
    allowed_tool_names = set(persona_tools) if persona_tools is not None else None

    # Registered builtin tools must be rebuilt by the config-gated injectors below.
    if req.func_tool is not None:
        tool_mgr = plugin_context.get_llm_tool_manager()
        for tool in list(req.func_tool):
            wrapped_tool = getattr(tool, "_wrapped", tool)
            if tool_mgr.is_builtin_tool(tool.name) and not isinstance(
    ...
    active_config = plugin_context.get_config(umo=event.unified_msg_origin)
    image_tool_statuses = get_builtin_tool_config_statuses(
        "generate_image",
        [{"conf_id": "current", "conf_name": "current", "config": active_config}],
    )
    if image_tool_statuses and image_tool_statuses[0]["enabled"]:
        if req.func_tool is None:
            req.func_tool = ToolSet()
        tool_mgr = plugin_context.get_llm_tool_manager()
        image_tool = tool_mgr.get_builtin_tool(ImageGenerationTool)
        if image_tool.active:
            req.func_tool.add_tool(tool_mgr.guard_tool(image_tool))
    ...
    authorized_tool_set = ToolSet()
    if req.func_tool is not None:
        for tool in req.func_tool:
            authorized_tool_set.add_tool(tool)
    ...
    reset_coro = agent_runner.reset(
        ...
        authorized_tool_set=authorized_tool_set,
    )
