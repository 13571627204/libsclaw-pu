...
def _is_ignored_zip_entry(name: str) -> bool:
    ...
    if parts[0] == "__MACOSX":
        return True
    # macOS Finder metadata that ends up in zips created on macOS.
    return Path(name).name in {".DS_Store"} or Path(name).name.startswith("._")


def _resolve_zip_skill_layout(
    file_names: list[str],
) -> tuple[bool, str, set[str]]:
    """Resolve how skill directories are laid out inside a zip archive.

    Returns ``(root_mode, skill_root_prefix, top_dirs)``:

    - ``root_mode``: the archive root itself is one skill (``SKILL.md`` at
      the top level of the zip).
    - ``skill_root_prefix``: wrapper directory prefix (e.g. ``"skills/"``)
      that groups multiple skill directories.
    - ``top_dirs``: archive entries to scan for skill directories. In wrapper
      mode these are the nested skill directory names; otherwise every
      top-level entry of the archive.
    """
    ...
    if not direct_skill_dirs and len(top_dirs) == 1:
        ...
        if nested_skill_dirs:
            return False, f"{wrapper_dir}/", nested_skill_dirs
    return False, "", top_dirs
...
class SkillManager:
    ...
    def install_skill_from_zip(
        ...
        *,
        ...
        """Install skill(s) from a zip archive and return the installed names.

        ``on_conflict`` overrides ``overwrite`` when provided:

        - ``"error"``: abort without installing anything if any skill already
          exists (same as ``overwrite=False``).
        - ``"skip"``: install only the skills that do not exist yet.
        - ``"overwrite"``: replace existing skills (same as ``overwrite=True``).

        The return value joins the installed skill names with ", ". When all
        skills were skipped it is an empty string.
        """
        ...
        strategy = on_conflict if on_conflict is not None else (
            "overwrite" if overwrite else "error"
        )
        if strategy not in {"error", "skip", "overwrite"}:
            raise ValueError(
                "Invalid conflict strategy, must be error/skip/overwrite."
            )
        ...
        with zipfile.ZipFile(zip_path) as zf:
            ...
            root_mode, skill_root_prefix, top_dirs = _resolve_zip_skill_layout(
                file_names
            )

            if not root_mode and strategy == "error":
            ...
            with tempfile.TemporaryDirectory(dir=get_astrbot_temp_path()) as tmp_dir:
                ...
                if root_mode:
                    ...
                    if dest_dir.exists() and strategy == "skip":
                        skipped_skills.append(skill_name)
                    else:
                        if dest_dir.exists() and strategy == "error":
                            raise FileExistsError(
    ...
    def inspect_skills_in_zip(
        self,
        zip_path: str,
        *,
        skill_name_hint: str | None = None,
    ) -> list[dict]:
        """List the skills contained in a zip archive without installing them.

        Returns one ``{"name": str, "exists": bool}`` entry per detected skill
        directory, mirroring the naming rules of ``install_skill_from_zip``.
        """
        ...
        with zipfile.ZipFile(zip_path) as zf:
            ...
            if root_mode:
                archive_hint = _normalize_skill_name(
                    archive_skill_name or Path(zip_path).stem
                )
                if not archive_hint or not _SKILL_NAME_RE.fullmatch(archive_hint):
                    raise ValueError("Invalid skill name.")
                skill_names.append(archive_hint)
            else:
                for archive_root_name in sorted(top_dirs):
                    if (
                        f"{skill_root_prefix}{archive_root_name}/SKILL.md"
                    ...
                    if archive_root_name in {".", "..", ""}:
                        continue
                    candidate_name = _normalize_skill_name(archive_root_name)
                    if not candidate_name or not _SKILL_NAME_RE.fullmatch(
                        candidate_name
                    ):
                        continue
                    if (
                        ...
                        skill_names.append(archive_skill_name)
                    else:
                        skill_names.append(candidate_name)

            return [
                {"name": name, "exists": (Path(self.skills_root) / name).exists()}
