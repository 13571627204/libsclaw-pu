...
class FunctionToolExecutor(BaseFunctionToolExecutor[AstrAgentContext]):
    ...
    def _build_handoff_toolset(
        ...
        """Build a subagent toolset within the configured runtime boundary.

        Args:
            run_context: Active agent execution context.
            tools: Tool allowlist, or ``None`` to allow all configured tools.

        Returns:
            The filtered toolset, or ``None`` when no tools are available.
        """
        ...
        if tools is None:
            ...
            for builtin_tool in tool_mgr.iter_builtin_tools():
                runtime_tool = runtime_computer_tools.get(builtin_tool.name)
                if runtime_tool is not None:
                    if runtime_tool.active:
                        toolset.add_tool(tool_mgr.guard_tool(runtime_tool))
                    continue

                config_statuses = get_builtin_tool_config_statuses(
                    builtin_tool.name,
                    config_entries,
                )
                if (
                    config_statuses
                    and config_statuses[0]["enabled"]
                    and builtin_tool.active
                ):
                    toolset.add_tool(tool_mgr.guard_tool(builtin_tool))
        ...
        for tool_name_or_obj in tools:
            if isinstance(tool_name_or_obj, str):
                if tool_mgr.is_builtin_tool(tool_name_or_obj):
                    runtime_tool = runtime_computer_tools.get(tool_name_or_obj)
                    if runtime_tool and runtime_tool.active:
                        toolset.add_tool(tool_mgr.guard_tool(runtime_tool))
                        continue
                    ...
                    if config_statuses and config_statuses[0]["enabled"]:
                        builtin_tool = tool_mgr.get_builtin_tool(tool_name_or_obj)
                        if builtin_tool.active:
                            toolset.add_tool(tool_mgr.guard_tool(builtin_tool))
                    continue

                registered_tool = tool_mgr.get_func(tool_name_or_obj)
                if registered_tool and registered_tool.active:
                    toolset.add_tool(tool_mgr.guard_tool(registered_tool))
            elif isinstance(tool_name_or_obj, FunctionTool):
                if tool_mgr.is_builtin_tool(tool_name_or_obj.name):
                    runtime_tool = runtime_computer_tools.get(tool_name_or_obj.name)
                    if runtime_tool and runtime_tool.active:
                        toolset.add_tool(tool_mgr.guard_tool(runtime_tool))
                        continue
                    ...
                    if config_statuses and config_statuses[0]["enabled"]:
                        builtin_tool = tool_mgr.get_builtin_tool(tool_name_or_obj.name)
                        if builtin_tool.active:
                            toolset.add_tool(tool_mgr.guard_tool(builtin_tool))
                    continue

                if tool_name_or_obj.active:
                    toolset.add_tool(tool_mgr.guard_tool(tool_name_or_obj))
        return None if toolset.empty() else toolset
    ...
    async def _execute_local(
        ...
        **tool_args,
        ...
        tool_execution_timeout = getattr(tool, "execution_timeout", None)
        timeout = (
            tool_call_timeout
