...
class LarkMessageEvent(AstrMessageEvent):
    ...
    # Lark rejects a post message whose serialized content exceeds ~30 KB.
    # Leave headroom for the JSON envelope and non-text segments.
    ...
    @classmethod
    def _split_text_by_bytes(cls, text: str) -> list[str]:
        """Split text so each UTF-8 encoded chunk fits within MAX_TEXT_BYTES.

        Prefers breaking at a newline, then at a space, to avoid cutting
        words or markdown structures apart. Never splits a multi-byte
        character in half.
        ...
        Returns:
            Chunks whose UTF-8 length each fits within MAX_TEXT_BYTES.
            Returns [] for empty input.
        """
        if not text:
            return []
        if len(text.encode("utf-8")) <= cls.MAX_TEXT_BYTES:
            return [text]
        ...
        while remaining:
            ...
            # Decode back with errors="ignore" to drop a trailing partial
            # character, so the cut always lands on a character boundary.
            segment = encoded[: cls.MAX_TEXT_BYTES].decode("utf-8", errors="ignore")
            # Cut *after* the separator so it stays with the preceding chunk;
            # dropping it would corrupt code blocks, tables and nested lists.
            split_at = segment.rfind("\n") + 1
            if split_at <= 1:
                split_at = segment.rfind(" ") + 1
            if split_at <= 1:
    ...
    async def _convert_to_lark(message: MessageChain, lark_client: lark.Client) -> list:
        ...
        for comp in message.chain:
            if isinstance(comp, Plain):
                ...
                # Rebind instead of clearing: _stage was appended by reference,
                # so clearing it in place would wipe the text already queued.
                if _stage:
                    ret.append(_stage)
                    _stage = []
    ...
    async def send_message_chain(
        ...
        if other_components:
            ...
            async def _flush_buffer() -> None:
                ...
                # Oversized text must be split before conversion, otherwise
                # Lark rejects the whole message and the content is lost.
                expanded: list = []
                for comp in buffered_components:
                    if isinstance(comp, Plain) and comp.text:
                        pieces = LarkMessageEvent._split_text_by_bytes(comp.text)
                        if len(pieces) > 1:
                            logger.warning(
                                f"[Lark] 文本超出单条消息上限，已拆分为 {len(pieces)} 条发送",
                            )
                        expanded.extend(Plain(piece) for piece in pieces)
                    else:
                        expanded.append(comp)
                buffered_components = []

                # Group components into batches that each stay under the limit.
                batches: list[list] = []
                current: list = []
                current_bytes = 0
                for comp in expanded:
                    comp_bytes = (
                        len(comp.text.encode("utf-8"))
                        if isinstance(comp, Plain) and comp.text
                        else 0
                    )
                    if current and current_bytes + comp_bytes > (
                        LarkMessageEvent.MAX_TEXT_BYTES
                    ):
                        batches.append(current)
                        current = []
                        current_bytes = 0
                    current.append(comp)
                    current_bytes += comp_bytes
                if current:
                    batches.append(current)

                for batch in batches:
                    pending_chain = MessageChain()
                    pending_chain.chain = batch
                    ...
                    if res:  # 只在有内容时发送
                        ...
                        await LarkMessageEvent._send_im_message(
                            lark_client,
                            # ensure_ascii=False keeps CJK as-is; escaping would
                            # triple the byte size and blow the message limit.
    ...
    async def _send_file_message(
        ...
        Args:
            ...
            receive_id_type: 接收者ID类型（用于主动发送）
        """
        # Use the async accessor so url-only components get downloaded first;
        # the sync `.file` property returns "" inside a running event loop.
        try:
            file_path = await file_comp.get_file()
        except Exception as e:
            logger.error(f"[Lark] 无法获取文件({file_comp.name}): {e}")
            return

        if not file_path:
            logger.error(f"[Lark] 无法解析文件路径: {file_comp.name}")
            return
