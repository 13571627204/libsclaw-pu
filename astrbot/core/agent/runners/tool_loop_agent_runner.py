...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    async def reset(
        ...
        authorized_tool_set: ToolSet | None = None,
        **kwargs: T.Any,
    ) -> None:
        self.req = request
        ...
        # Freeze canonical tools before runtime hooks can mutate the request.
        self._authorized_tool_set = ToolSet()
        authorization_source = (
            authorized_tool_set
            if authorized_tool_set is not None
            else self.req.func_tool
        )
        if authorization_source is not None:
            for tool in authorization_source:
                if getattr(tool, "active", True):
                    self._authorized_tool_set.add_tool(tool)
        self._authorized_provider_tool_set = self._authorized_tool_set
        ...
        if tool_schema_mode == "skills_like":
            tool_set = self._authorized_tool_set
            ...
            self._authorized_provider_tool_set = light_set
            # MODIFIE the req.func_tool to use light tool schemas
            self.req.func_tool = ToolSet(list(light_set.tools))
    ...
    def _func_tool_for_provider(self) -> ToolSet | None:
        if self.req.func_tool is None:
            return None
        if self._authorized_provider_tool_set.empty():
            return None
        modalities = self.provider.provider_config.get("modalities", None)
        if isinstance(modalities, list) and modalities and "tool_use" not in modalities:
            ...
            return None
        current_tool_names = set(self.req.func_tool.names())
        effective_tool_set = ToolSet()
        for tool in self._authorized_provider_tool_set:
            if tool.name in current_tool_names:
                effective_tool_set.add_tool(tool)
        return None if effective_tool_set.empty() else effective_tool_set
    ...
    async def _handle_function_tools(
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            try:
                current_tool_names = (
                    set(req.func_tool.names()) if req.func_tool is not None else set()
                )
                func_tool = (
                    self._authorized_tool_set.get_tool(func_tool_name)
                    if func_tool_name in current_tool_names
                    else None
                )
                available_tools = [
                    name
                    for name in self._authorized_tool_set.names()
                    if name in current_tool_names
                ]
    ...
    async def _resolve_tool_exec(
        ...
        full_tool_set = self._authorized_tool_set
