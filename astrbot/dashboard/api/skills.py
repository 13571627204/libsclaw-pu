...
def _conflict_strategy(request: Request) -> str:
    value = str(request.query_params.get("on_conflict", "error") or "error").strip()
    if value not in {"error", "skip", "overwrite"}:
...
async def upload_skills_batch(
    ...
    async def _operation():
        ...
        return await service.batch_upload_skills(
            files.getlist("files"),
            on_conflict=_conflict_strategy(request),
        )
...
async def inspect_skills_archives(
    request: Request,
    _auth: AuthContext = Depends(require_skill_scope),
    service: SkillsService = Depends(get_service),
):
    async def _operation():
        _, files = await multipart_parts(request)
        return await service.inspect_uploaded_skills(files.getlist("files"))
...
@legacy_router.post("/inspect")
async def inspect_dashboard_skills(
    request: Request,
    _username: str = Depends(require_dashboard_user),
    service: SkillsService = Depends(get_service),
):
