...
class SkillsService:
    ...
    async def batch_upload_skills(
        ...
        for file in file_list:
            ...
            try:
                ...
                entries = skill_mgr.inspect_skills_in_zip(
                    temp_path,
                    skill_name_hint=Path(filename).stem,
                )
                if not entries:
                    raise ValueError("No valid SKILL.md found in the zip archive.")

                existing_names = [e["name"] for e in entries if e["exists"]]
                new_names = [e["name"] for e in entries if not e["exists"]]
                ...
                if on_conflict != "error":
                    install_kwargs["on_conflict"] = on_conflict
                try:
                    skill_name = skill_mgr.install_skill_from_zip(
                        temp_path,
                        **install_kwargs,
                    ...
                    # Fallback for installers without the newer kwargs.
                    install_kwargs.pop("on_conflict", None)
                    install_kwargs.pop("skill_name_hint", None)
                    skill_name = skill_mgr.install_skill_from_zip(
                        temp_path,
                        **install_kwargs,
                    )
                ...
                if on_conflict == "skip" and not new_names:
                    skipped.append(
                        {
                            "filename": filename,
                            "name": ", ".join(existing_names),
                ...
                succeeded.append(
                    {
                        "filename": filename,
                        "name": skill_name,
                        "names": all_names if on_conflict != "skip" else new_names,
                        "skipped": existing_names if on_conflict == "skip" else [],
                    }
                )
    ...
    async def inspect_uploaded_skills(
        self,
        file_list: list[Any],
    ) -> SkillsOperationResult:
        """Pre-check uploaded archives without installing anything.

        Returns per-file skill inventories with existence flags so the
        dashboard can ask the user how to handle duplicates.
        """
        if not file_list:
            raise SkillsServiceError("No files provided")
        ...
        for file in file_list:
            filename = os.path.basename(file.filename or "unknown.zip")
            entry = {"filename": filename, "skills": [], "error": None}
            temp_path = None
            ...
            try:
                ...
                skills = skill_mgr.inspect_skills_in_zip(
                    temp_path,
                    skill_name_hint=Path(filename).stem,
                )
                if not skills:
                    raise SkillsServiceError(
                        "No valid SKILL.md found in the zip archive."
                    )
                entry["skills"] = skills
            except Exception as exc:
                entry["error"] = str(exc)
        ...
        return SkillsOperationResult(
            data={"files": files_report},
            message=f"Inspected {len(files_report)} file(s).",
        )
