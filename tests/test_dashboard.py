...
async def test_md5_login_failure_includes_upgrade_faq_hint(
    ...
    try:
        ...
        assert "请通过命令行重置密码" in data["message"]
    finally:
        await _restore_dashboard_password_state(
            core_lifecycle_td,
            original_dashboard_config,
...
@pytest.mark.asyncio
async def test_do_update_disabled(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
):
    """在线更新已在此发行版中禁用：/api/update/do 应返回错误且不触发任何下载。"""
    test_client = app.test_client()

    response = await test_client.post(
        "/api/update/do",
        headers=authenticated_header,
        json={"version": "latest", "reboot": False},
    )
    data = await response.get_json()
    assert data["status"] == "error"
    assert "已在此发行版中禁用" in data["message"]
...
async def test_batch_upload_skills_accepts_zip_files(
    ...
    def _fake_inspect_skills_in_zip(self, zip_path: str, *, skill_name_hint=None):
        _ = self, skill_name_hint
        assert zip_path.endswith(".zip")
        return [{"name": "demo_skill", "exists": False}]
    ...
    monkeypatch.setattr(
        "astrbot.dashboard.services.skills_service.SkillManager.inspect_skills_in_zip",
        _fake_inspect_skills_in_zip,
    )
...
def _setup_skills_upload_env(monkeypatch, tmp_path):
    data_dir = tmp_path / "data"
    skills_dir = tmp_path / "skills"
    temp_dir = tmp_path / "temp"
    for directory in (data_dir, skills_dir, temp_dir):
        directory.mkdir()
...
def _make_conflicting_skills_archive() -> io.BytesIO:
    archive = io.BytesIO()
    with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("skills/docx/SKILL.md", "---\nname: docx\n---\nnew docx")
        zf.writestr("skills/pdf/SKILL.md", "---\nname: pdf\n---\nnew pdf")
        zf.writestr("skills/.DS_Store", "junk")
        zf.writestr("__MACOSX/skills/._docx", "junk")
    archive.seek(0)
    return archive


@pytest.mark.asyncio
async def test_batch_upload_skills_skip_existing_conflicts(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
    monkeypatch,
    tmp_path,
):
    skills_dir = _setup_skills_upload_env(monkeypatch, tmp_path)
    (skills_dir / "docx").mkdir()
    (skills_dir / "docx" / "SKILL.md").write_text("old docx")

    test_client = app.test_client()

    response = await test_client.post(
        "/api/skills/batch-upload?on_conflict=skip",
        headers=authenticated_header,
        files={
            "files": FileStorage(
                stream=_make_conflicting_skills_archive(),
                filename="skills.zip",
                content_type="application/zip",
            ),
        },
    ...
    assert (skills_dir / "docx" / "SKILL.md").read_text() == "old docx"
    assert (skills_dir / "pdf" / "SKILL.md").read_text().endswith("new pdf")


@pytest.mark.asyncio
async def test_batch_upload_skills_overwrite_existing_conflicts(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
    monkeypatch,
    tmp_path,
    ...
    response = await test_client.post(
        "/api/skills/batch-upload?on_conflict=overwrite",
        headers=authenticated_header,
        files={
            "files": FileStorage(
                stream=_make_conflicting_skills_archive(),
    ...
    assert data["data"]["failed"] == []
    assert (skills_dir / "docx" / "SKILL.md").read_text().endswith("new docx")
    assert (skills_dir / "pdf" / "SKILL.md").exists()


@pytest.mark.asyncio
async def test_batch_upload_skills_rejects_invalid_conflict_strategy(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
    monkeypatch,
    tmp_path,
):
    _setup_skills_upload_env(monkeypatch, tmp_path)

    test_client = app.test_client()

    response = await test_client.post(
        "/api/skills/batch-upload?on_conflict=bogus",
        headers=authenticated_header,
        files={
            "files": FileStorage(
                stream=_make_conflicting_skills_archive(),
    ...
    data = await response.get_json()
    assert data["status"] == "error"
    assert "Invalid conflict strategy" in data["message"]


@pytest.mark.asyncio
async def test_inspect_skills_reports_existing_conflicts(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
    monkeypatch,
    tmp_path,
    ...
    data = await response.get_json()
    assert data["status"] == "ok"
    files_report = data["data"]["files"]
    assert len(files_report) == 1
    assert files_report[0]["filename"] == "skills.zip"
    assert files_report[0]["error"] is None
    assert files_report[0]["skills"] == [
        {"name": "docx", "exists": True},
        {"name": "pdf", "exists": False},
    ]
    # Inspect must not install anything.
    assert not (skills_dir / "pdf").exists()


@pytest.mark.asyncio
async def test_inspect_skills_reports_invalid_archive(
    app: FastAPIAppAdapter,
    authenticated_header: dict,
    monkeypatch,
    tmp_path,
    ...
    assert data["status"] == "ok"
    files_report = data["data"]["files"]
    assert files_report[0]["filename"] == "broken.zip"
    assert files_report[0]["skills"] == []
    assert "not a valid zip archive" in files_report[0]["error"]
...
async def test_batch_upload_skills_partial_success(
    ...
    def _fake_inspect_skills_in_zip(self, zip_path: str, *, skill_name_hint=None):
        _ = self, skill_name_hint
        if "ok_skill" in zip_path:
            return [{"name": "ok_skill", "exists": False}]
        raise RuntimeError("install failed")
