...
class CapturingSequenceProvider(MockProvider):
    """Return scripted responses and capture exact tools sent to the model."""

    def __init__(self, responses: list[LLMResponse]):
        super().__init__()
        self.responses = responses
        self.tool_snapshots: list[list[FunctionTool] | None] = []

    async def text_chat(self, **kwargs) -> LLMResponse:
        """Capture a stable tool snapshot before returning the next response.

        Args:
            **kwargs: Provider request fields, including the current tool set.

        Returns:
            The next scripted LLM response.
        """
        func_tool = kwargs.get("func_tool")
        self.tool_snapshots.append(None if func_tool is None else list(func_tool.tools))
...
class ToolMutatingHooks(MockHooks):
    """Simulate plugins that mutate the request tool set from agent hooks."""

    def __init__(
        self,
        request: ProviderRequest,
        *,
        begin_tool: FunctionTool | None = None,
        end_tool: FunctionTool | None = None,
    ):
        super().__init__()
    ...
    async def on_agent_begin(self, run_context):
        """Add or replace a tool after the runner authorization is initialized.

        Args:
            run_context: Current agent execution context.
        """
        await super().on_agent_begin(run_context)
        if self.begin_tool is not None:
            assert self.request.func_tool is not None
            self.request.func_tool.add_tool(self.begin_tool)

    async def on_tool_end(self, run_context, tool, tool_args, tool_result):
        """Add or replace a tool after an authorized tool completes.

        Args:
            run_context: Current agent execution context.
            tool: Tool that completed.
            tool_args: Arguments passed to the tool.
            tool_result: Final result returned by the tool.
        """
        await super().on_tool_end(run_context, tool, tool_args, tool_result)
        if self.end_tool is not None:
            assert self.request.func_tool is not None
            self.request.func_tool.add_tool(self.end_tool)


class ToolRemovingHooks(MockHooks):
    """Simulate a hook that narrows the current request tool set."""

    def __init__(self, request: ProviderRequest, tool_name: str):
        super().__init__()
        self.request = request
        self.tool_name = tool_name

    async def on_agent_begin(self, run_context):
        """Remove one tool before the first provider request.
        ...
        await super().on_agent_begin(run_context)
        assert self.request.func_tool is not None
        self.request.func_tool.remove_tool(self.tool_name)


class RecordingToolExecutor:
    """Record exact tool objects that reach the execution boundary."""

    def __init__(self):
        self.executed_tools: list[FunctionTool] = []

    def execute(self, tool, run_context, **tool_args):
        """Record a tool and return a minimal successful result stream.

        Args:
            tool: Tool selected for execution.
            run_context: Current agent execution context.
            **tool_args: Validated tool arguments.

        Returns:
            An async generator yielding one successful tool result.
        """
        del run_context, tool_args
        ...
        async def generator():
            ...
            yield CallToolResult(
                content=[TextContent(type="text", text="authorized result")]
            )
...
def _make_named_tool(name: str) -> FunctionTool:
    """Create a minimal active function tool for authorization tests.

    Args:
        name: Tool name exposed to the model.

    Returns:
        A function tool with a no-argument schema.
    """
    return FunctionTool(
        name=name,
...
@pytest.mark.asyncio
async def test_on_agent_begin_cannot_expand_authorized_toolset():
    """Tools injected by on_agent_begin must not reach the provider."""
    allowed_tool = _make_named_tool("allowed_tool")
    injected_tool = _make_named_tool("injected_tool")
    request = ProviderRequest(
        prompt="test strict authorization",
        func_tool=ToolSet([allowed_tool]),
        contexts=[],
    )
    ...
    await runner.reset(
        ...
        agent_hooks=ToolMutatingHooks(request, begin_tool=injected_tool),
    )

    async for _ in runner.step():
        pass

    assert provider.tool_snapshots[0] is not None
    assert [tool.name for tool in provider.tool_snapshots[0]] == [allowed_tool.name]
    assert provider.tool_snapshots[0][0] is allowed_tool


@pytest.mark.asyncio
async def test_on_agent_begin_cannot_replace_authorized_tool_with_same_name():
    """A hook cannot replace an authorized implementation under the same name."""
    authorized_tool = _make_named_tool("allowed_tool")
    replacement_tool = _make_named_tool("allowed_tool")
    request = ProviderRequest(
        prompt="test same-name replacement",
        func_tool=ToolSet([authorized_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
            LLMResponse(
                role="assistant",
                tools_call_name=[authorized_tool.name],
                tools_call_args=[{}],
                tools_call_ids=["call_allowed"],
                usage=TokenUsage(input_other=10, output=5),
            ),
            LLMResponse(
                role="assistant",
    ...
    await runner.reset(
        ...
        agent_hooks=ToolMutatingHooks(request, begin_tool=replacement_tool),
    )

    async for _ in runner.step_until_done(3):
        pass

    assert len(provider.tool_snapshots) == 2
    assert all(snapshot is not None for snapshot in provider.tool_snapshots)
    assert all(
        snapshot[0] is authorized_tool
        for snapshot in provider.tool_snapshots
        if snapshot is not None
    )
    assert executor.executed_tools == [authorized_tool]


@pytest.mark.asyncio
async def test_on_tool_end_cannot_expand_authorized_toolset():
    """Tools injected by on_tool_end must be removed before the next model call."""
    allowed_tool = _make_named_tool("allowed_tool")
    injected_tool = _make_named_tool("injected_tool")
    request = ProviderRequest(
        prompt="test post-tool authorization",
        func_tool=ToolSet([allowed_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
            LLMResponse(
                role="assistant",
                tools_call_name=[allowed_tool.name],
                tools_call_args=[{}],
                tools_call_ids=["call_allowed"],
                usage=TokenUsage(input_other=10, output=5),
            ),
    ...
    await runner.reset(
        ...
        run_context=ContextWrapper(context=None),
        tool_executor=RecordingToolExecutor(),
        agent_hooks=ToolMutatingHooks(request, end_tool=injected_tool),
    )
    ...
    assert len(provider.tool_snapshots) == 2
    assert [
        [tool.name for tool in snapshot] if snapshot is not None else None
        for snapshot in provider.tool_snapshots
    ] == [[allowed_tool.name], [allowed_tool.name]]


@pytest.mark.asyncio
async def test_execution_rejects_tool_injected_after_authorization_snapshot():
    """A forged model call cannot execute a tool outside the initial snapshot."""
    allowed_tool = _make_named_tool("allowed_tool")
    injected_tool = _make_named_tool("injected_tool")
    request = ProviderRequest(
        prompt="test execution boundary",
        func_tool=ToolSet([allowed_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
            LLMResponse(
                role="assistant",
                tools_call_name=[injected_tool.name],
                tools_call_args=[{}],
                tools_call_ids=["call_injected"],
                usage=TokenUsage(input_other=10, output=5),
            ),
            LLMResponse(
                role="assistant",
    ...
    assert executor.executed_tools == []
    assert request.tool_calls_result
    tool_result = str(request.tool_calls_result[0].tool_calls_result[0].content).lower()
    assert any(
        marker in tool_result
        for marker in ("not found", "not available", "unauthorized")
    )


@pytest.mark.asyncio
async def test_pre_reset_request_mutation_cannot_expand_build_authorization():
    """A pre-reset request mutation cannot enter the build authorization snapshot."""
    allowed_tool = _make_named_tool("allowed_tool")
    injected_tool = _make_named_tool("injected_tool")
    request = ProviderRequest(
        prompt="test pre-reset authorization",
        func_tool=ToolSet([allowed_tool, injected_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
    ...
    await runner.reset(
        ...
        authorized_tool_set=ToolSet([allowed_tool]),
    )
...
@pytest.mark.asyncio
async def test_hook_can_narrow_authorized_toolset():
    """A hook may remove an authorized tool without being restored by the snapshot."""
    kept_tool = _make_named_tool("kept_tool")
    removed_tool = _make_named_tool("removed_tool")
    request = ProviderRequest(
        prompt="test authorization narrowing",
        func_tool=ToolSet([kept_tool, removed_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
    ...
    await runner.reset(
        ...
        run_context=ContextWrapper(context=None),
        tool_executor=RecordingToolExecutor(),
        agent_hooks=ToolRemovingHooks(request, removed_tool.name),
    )
    ...
    assert provider.tool_snapshots[0] is not None
    assert [tool.name for tool in provider.tool_snapshots[0]] == [kept_tool.name]


@pytest.mark.asyncio
async def test_max_step_final_request_cannot_execute_forged_tool_call():
    """The no-tool final request must not execute a forged tool call."""
    allowed_tool = _make_named_tool("allowed_tool")
    request = ProviderRequest(
        prompt="test max-step execution boundary",
        func_tool=ToolSet([allowed_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
        [
            LLMResponse(
                ...
                tools_call_name=[allowed_tool.name],
                tools_call_args=[{}],
                tools_call_ids=["call_before_limit"],
                usage=TokenUsage(input_other=10, output=5),
            ),
            LLMResponse(
                role="assistant",
                tools_call_name=[allowed_tool.name],
                tools_call_args=[{}],
                tools_call_ids=["call_after_limit"],
                usage=TokenUsage(input_other=10, output=5),
            ),
        ]
    )
    ...
    assert executor.executed_tools == [allowed_tool]


@pytest.mark.asyncio
async def test_skills_like_hook_injection_cannot_expand_schema_or_execution():
    """skills_like mode must expose and execute only the initial canonical tool."""
    authorized_tool = _make_named_tool("allowed_tool")
    injected_tool = _make_named_tool("injected_tool")
    request = ProviderRequest(
        prompt="test skills-like authorization",
        func_tool=ToolSet([authorized_tool]),
        contexts=[],
    )
    provider = CapturingSequenceProvider(
    ...
    assert [
        [tool.name for tool in snapshot] if snapshot is not None else None
        for snapshot in provider.tool_snapshots
    ] == [[authorized_tool.name], [authorized_tool.name], [authorized_tool.name]]
    assert provider.tool_snapshots[0] is not None
    assert provider.tool_snapshots[0][0].handler is None
    assert provider.tool_snapshots[0][0].parameters == {
        "type": "object",
        "properties": {},
    }
    assert executor.executed_tools == [authorized_tool]
