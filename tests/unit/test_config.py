...
class TestAiConfigVisibility:
    """Tests for hidden AI configuration metadata."""

    def test_requested_ai_config_sections_and_fields_are_hidden(self):
        """Test that screenshot-scoped AI settings are hidden from the WebUI."""
        sections = CONFIG_METADATA_3["ai_group"]["metadata"]

        assert "hint" not in sections["agent_runner"]
        assert sections["websearch"]["invisible"] is True
        ...
        for section, fields in hidden_fields.items():
            items = sections[section]["items"]
            for field in fields:
                assert items[field]["invisible"] is True

        collapsed_items = [
            item
            for item in sections["others"]["items"].values()
            if item.get("collapsed")
        ]
        assert collapsed_items
        assert all(item.get("invisible") is True for item in collapsed_items)

    def test_neighboring_ai_config_fields_remain_visible(self):
        """Test that fields outside the screenshots remain visible."""
        sections = CONFIG_METADATA_3["ai_group"]["metadata"]
        visible_fields = {
            "agent_runner": [
            ...
            "ai": ["provider_settings.default_provider_id"],
            "knowledgebase": ["kb_names"],
            "others": ["provider_settings.streaming_response"],
        }
        for section, fields in visible_fields.items():
            items = sections[section]["items"]
            for field in fields:
                assert items[field].get("invisible") is not True


class TestPlatformConfigVisibility:
    """Tests for hidden platform configuration metadata."""

    def test_requested_platform_sections_and_fields_are_hidden(self):
        """Test that screenshot-scoped platform settings are hidden."""
        sections = CONFIG_METADATA_3["platform_group"]["metadata"]

        for section in ["whitelist", "rate_limit", "content_safety", "t2i"]:
            assert sections[section]["invisible"] is True
    ...
    def test_neighboring_platform_fields_remain_visible(self):
        """Test that platform fields outside the screenshots remain visible."""
        sections = CONFIG_METADATA_3["platform_group"]["metadata"]
        visible_fields = [
            "admins_id",
        ...
        for field in visible_fields:
            assert sections["general"]["items"][field].get("invisible") is not True


def test_plugin_config_group_is_hidden():
    """Test that only the plugin configuration group is hidden."""
    assert CONFIG_METADATA_3["plugin_group"]["invisible"] is True
    converted = ConfigMetadataI18n.convert_to_i18n_keys(CONFIG_METADATA_3)
    assert converted["plugin_group"]["invisible"] is True
    assert "plugin_set" in converted["plugin_group"]["metadata"]["plugin"]["items"]
    for group in ["ai_group", "platform_group", "ext_group"]:
        assert converted[group].get("invisible") is not True


def test_requested_extension_config_fields_are_hidden():
    """Test that screenshot-scoped extension settings are hidden."""
    sections = ConfigMetadataI18n.convert_to_i18n_keys(CONFIG_METADATA_3)["ext_group"][
        "metadata"
    ]
    hidden_fields = {
        "segmented_reply": [
