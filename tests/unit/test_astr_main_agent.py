...
def mock_context():
    ...
    tool_mgr.guard_tool.side_effect = lambda tool: tool
...
class TestApplyKb:
    ...
    @pytest.mark.asyncio
    async def test_apply_kb_skips_retrieval_when_search_tool_is_not_allowed(
        self,
        mock_event,
        mock_context,
    ):
        """Do not retrieve knowledge when the persona excludes KB search."""
...
class TestEnsurePersonaAndSkills:
    ...
    async def test_ensure_tools_from_persona(self, mock_event, mock_context):
        ...
        guarded_tool = MagicMock()
        guarded_tool.name = "test_tool"
        guarded_tool.active = True
        tmgr.is_builtin_tool.return_value = False
        tmgr.get_func.return_value = mock_tool
        tmgr.guard_tool.side_effect = None
        tmgr.guard_tool.return_value = guarded_tool
        ...
        assert req.func_tool is not None
        assert req.func_tool.get_tool("test_tool") is guarded_tool
        tmgr.guard_tool.assert_called_once_with(mock_tool)

    @pytest.mark.asyncio
    async def test_persona_empty_tools_removes_late_builtin_tools(
        ...
        mock_context.get_config.return_value = {
            ...
            "subagent_orchestrator": {
                "main_enable": True,
                "agents": [{"enabled": True, "persona_id": "subagent"}],
            },
        }
        mock_context.persona_manager.get_persona_v3_by_id.return_value = {
            "name": "subagent",
            "tools": None,
        }
        tool_mgr = mock_context.get_llm_tool_manager.return_value
        tool_mgr.guard_tool.side_effect = lambda tool: tool
        handoff = HandoffTool(Agent(name="planner"))
        tool_mgr.is_builtin_tool.side_effect = lambda name: (
            name
            not in {
                "astr_persona_memory",
                handoff.name,
    ...
    @pytest.mark.asyncio
    async def test_persona_empty_tools_removes_local_runtime_builtin_tools(
        self, mock_event, mock_context, mock_provider
    ):
        module = ama
        persona = {"name": "locked", "prompt": "No tools.", "tools": []}
    ...
    @pytest.mark.asyncio
    async def test_persona_selected_builtin_tools_filter_late_and_inactive_tools(
        self, mock_event, mock_context, mock_provider
    ):
        module = ama
        persona = {
            "name": "reader",
            "prompt": "Read knowledge base pages.",
            "tools": [
                "astr_kb_read_page",
                "web_search_baidu",
        ...
        mock_context.persona_manager.resolve_selected_persona = AsyncMock(
            return_value=("reader", persona, None, False)
        )
        ...
        def get_builtin_tool(tool_class):
            tool = tool_class()
            if tool.name == "web_search_baidu":
                tool.active = False
            return tool
        ...
        req = ProviderRequest(
            prompt="hello",
            func_tool=ToolSet([module.ExecuteShellTool(), module.BaiduWebSearchTool()]),
        )
        req.conversation = MagicMock(persona_id="reader", history="[]")
        ...
        try:
            assert result.provider_request.func_tool is not None
            assert result.provider_request.func_tool.names() == ["astr_kb_read_page"]
        finally:
            if result.reset_coro:
                result.reset_coro.close()

    @pytest.mark.asyncio
    async def test_configured_image_generation_tool_is_injected(
        self, mock_event, mock_context, mock_provider
    ):
        """Configured image generation is available to an allowed persona."""
        module = ama
        persona = {
            "name": "artist",
            "prompt": "Create images when requested.",
            "tools": ["generate_image"],
        }
        mock_context.persona_manager.resolve_selected_persona = AsyncMock(
            return_value=("artist", persona, None, False)
        )
        mock_event.platform_meta.support_proactive_message = False
        provider_settings = {
        ...
        mock_context.get_config.return_value = {"provider_settings": provider_settings}
        tool_mgr = mock_context.get_llm_tool_manager.return_value
        tool_mgr.guard_tool.side_effect = lambda tool: tool
        tool_mgr.is_builtin_tool.side_effect = lambda name: name == "generate_image"
        config = module.MainAgentBuildConfig(
            tool_call_timeout=60,
        ...
        req = ProviderRequest(prompt="draw a cat")
        req.conversation = MagicMock(persona_id="artist", history="[]")
        ...
        try:
            assert result.provider_request.func_tool is not None
            assert result.provider_request.func_tool.names() == ["generate_image"]
        finally:
            if result.reset_coro:
                result.reset_coro.close()

    @pytest.mark.asyncio
    async def test_persona_all_tools_keeps_every_active_configured_candidate(
        self, mock_event, mock_context, mock_provider
    ):
        """The all-tools mode keeps active plugin, builtin, memory, and handoff tools."""
        module = ama
        persona = {
            "name": "all-tools",
            "prompt": "Use every available tool.",
            "tools": None,
        }
        mock_context.persona_manager.resolve_selected_persona = AsyncMock(
            return_value=("all-tools", persona, None, False)
        )
        mock_event.platform_meta.support_proactive_message = False
        mock_context.get_config.return_value = {
            "provider_settings": {
        ...
        active_plugin_tool = FunctionTool(
            name="active_plugin_tool",
            description="Active plugin tool",
            parameters={"type": "object", "properties": {}},
            handler_module_path="plugin.active",
            active=True,
        )
        inactive_plugin_tool = FunctionTool(
            name="inactive_plugin_tool",
            description="Inactive plugin tool",
            parameters={"type": "object", "properties": {}},
            handler_module_path="plugin.inactive",
            active=False,
        )
        tool_mgr = mock_context.get_llm_tool_manager.return_value
        tool_mgr.get_full_tool_set.return_value = ToolSet(
            [active_plugin_tool, inactive_plugin_tool]
        )
        tool_mgr.guard_tool.side_effect = lambda tool: tool
        tool_mgr.is_builtin_tool.side_effect = lambda name: name == "web_search_baidu"
        handoff = HandoffTool(Agent(name="planner"))
        mock_context.subagent_orchestrator = MagicMock(handoffs=[handoff])
        config = module.MainAgentBuildConfig(
            tool_call_timeout=60,
        ...
        req = ProviderRequest(prompt="hello")
        req.conversation = MagicMock(persona_id="all-tools", history="[]")
        ...
        try:
            assert result.provider_request.func_tool is not None
            tool_names = set(result.provider_request.func_tool.names())
            assert {
                "active_plugin_tool",
                "web_search_baidu",
                "astr_persona_memory",
            ...
            assert "inactive_plugin_tool" not in tool_names
            assert all(tool.active for tool in result.provider_request.func_tool.tools)
        finally:
            if result.reset_coro:
                result.reset_coro.close()
