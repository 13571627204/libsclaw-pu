...
@pytest.mark.asyncio
async def test_v1_persona_create_preserves_explicit_empty_tools(
    asgi_client: httpx.AsyncClient,
    fake_core_lifecycle,
):
    persona_id = "no-tools"
    ...
    assert response.status_code == 200
    assert response.json()["data"]["persona"]["tools"] == []
    assert fake_core_lifecycle.persona_mgr.personas[persona_id].tools == []


@pytest.mark.asyncio
async def test_v1_persona_create_preserves_default_all_tools(
    asgi_client: httpx.AsyncClient,
    fake_core_lifecycle,
):
    persona_id = "all-tools"

    response = await asgi_client.post(
        "/api/v1/personas",
        json={
            "persona_id": persona_id,
            "system_prompt": "Use all configured tools.",
        },
        headers=_jwt_headers(),
    )

    assert response.status_code == 200
    assert response.json()["data"]["persona"]["tools"] is None
    assert fake_core_lifecycle.persona_mgr.personas[persona_id].tools is None
