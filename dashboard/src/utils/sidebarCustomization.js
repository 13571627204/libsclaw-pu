...
export function resolveSidebarItems(defaultItems, customization, options = {}) {
  ...
  const storedMainKeys = hasCustomization ? normalizeKeys(customization.mainItems || []) : [];
  const storedMoreKeys = hasCustomization ? normalizeKeys(customization.moreItems || []) : [];
  let mainKeys = hasCustomization ? [...storedMainKeys] : [...defaultMain];
  let moreKeys = hasCustomization ? [...storedMoreKeys] : [...defaultMore];

  if (hasCustomization) {
    const legacyNavigationKeys = new Set([
    ...
    const hasLegacyNavigation = [...storedMainKeys, ...storedMoreKeys]
      .some(title => legacyNavigationKeys.has(title));

    if (hasLegacyNavigation) {
      // Move former More children to the main section during legacy cleanup.
      const promotedKeys = new Set([
        'core.navigation.conversation',
        'core.navigation.cron',
        'core.navigation.console',
      ]);
      moreKeys = moreKeys.filter(title => {
        if (!promotedKeys.has(title)) return true;
        if (!mainKeys.includes(title)) mainKeys.push(title);
        return false;
      });
    }
  }
