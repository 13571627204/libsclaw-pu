...
export const skillApi = {
  ...
  uploadBatch(files: File[], onConflict?: 'error' | 'skip' | 'overwrite') {
    return typed<any>(
      openApiV1.uploadSkillsBatch({
        query: onConflict && onConflict !== 'error' ? { on_conflict: onConflict } : undefined,
        body: { files },
      }),
    );
  },
  inspect(files: File[]) {
    return typed<any>(openApiV1.inspectSkills({ body: { files } }));
  },
