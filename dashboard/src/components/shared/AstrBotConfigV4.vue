...
function shouldShowSection() {
  ...
  const hasVisibleItems = Object.entries(sectionItems).some(
    ([itemKey, itemMeta]) => !itemMeta?.invisible && shouldShowItem(itemMeta, itemKey)
  )
...
<template>
  ...
  <v-card v-if="shouldShowSection()" class="config-card"
    ...
    <v-card-text class="config-section" v-if="metadata[metadataKey]?.type === 'object'">
      ...
      <v-list-item-subtitle v-if="metadata[metadataKey]?.hint" class="config-hint">
